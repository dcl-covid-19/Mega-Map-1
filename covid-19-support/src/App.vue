...
export default {
  ...
  computed: {
    showSearchBar() {
      if (this.$route.name == 'AboutUs' || this.$route.name == 'BlmStatement' || this.$route.name == 'LegalSupport') {
        return false
      }
  ...
  watch: {
    $route(to) {
      ...
      if (to.path == '/') {
        ...
        if (!needs_name.includes(to.params.need) && !['/about-us', '/blm-statement', '/legal-support'].includes(to.path)) {
          this.$router.push('/')
        } else {
          this.initialSearch = false
          if (to.params.need) {
